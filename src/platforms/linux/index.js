...
const installDockerOnLinux = async () => {
  return asyncPipe(
    ...
    _ => {
      ...
      const isDockerComposeInstalled = checkStringForCharacter(
        whichDockerCompose,
        'docker-compose'
      )
      if (isDockerInstalled && isDockerComposeInstalled)
        return logSuccess('docker  and docker-compose installed successfully')
      !isDockerInstalled &&
        logError(
          'docker installation failed. Please visit https://docs.docker.com/install/linux/docker-ce/ubuntu/ to install'
        )
      !isDockerComposeInstalled &&
        logError(
          'docker compose installation failed. You can manually install from here https://docs.docker.com/compose/install/'
        )
      shell.exit(1)
    },
    _ => sleep(100)
